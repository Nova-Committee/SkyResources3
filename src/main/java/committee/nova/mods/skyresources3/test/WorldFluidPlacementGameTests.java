...
public final class WorldFluidPlacementGameTests {
    ...
    public static void fluidDropperPlacesSourceAndConsumesOneBucket(final GameTestHelper helper) {
        withDropperCapacity(INITIAL_FLUID_AMOUNT, () -> {
            ...
            helper.assertValueEqual(
                    FluidType.BUCKET_VOLUME,
                    dropper.getFluidHandler().getAmountAsInt(0),
    ...
    private static Player waterExtractorPlayer(final GameTestHelper helper) {
        ...
        final ResourceHandler<FluidResource> handler = ItemAccess
                .forPlayerInteraction(player, InteractionHand.MAIN_HAND)
                .oneByOne()
                .getCapability(Capabilities.Fluid.ITEM);
        helper.assertTrue(handler != null, "Water Extractor should expose a fluid capability");
        helper.assertValueEqual(
                INITIAL_FLUID_AMOUNT,
                insertFluid(handler, INITIAL_FLUID_AMOUNT),
    ...
    private static FluidDropperBlockEntity filledDropper(final GameTestHelper helper) {
        ...
        final FluidDropperBlockEntity dropper = helper.getBlockEntity(DROPPER_POS, FluidDropperBlockEntity.class);
        helper.assertValueEqual(
                INITIAL_FLUID_AMOUNT,
                insertFluid(dropper.getFluidHandler(), INITIAL_FLUID_AMOUNT),
                "Fluid Dropper should accept the test water"
        );
        return dropper;
    }

    private static int insertFluid(final ResourceHandler<FluidResource> handler, final int amount) {
        try (Transaction transaction = Transaction.openRoot()) {
            final int inserted = handler.insert(WATER, amount, transaction);
            transaction.commit();
            return inserted;
        }
    }
